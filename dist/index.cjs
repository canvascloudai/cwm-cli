...
var CONFIG_DIR = import_node_path.default.join(import_node_os.default.homedir(), ".cwm");
...
function saveConfig(updates) {
  ...
  import_node_fs.default.writeFileSync(CONFIG_FILE, JSON.stringify(merged, null, 2), "utf-8");
}
function resolveConfig(opts = {}) {
  const stored = loadConfig();
  const baseUrl = opts.baseUrl ?? stored.baseUrl ?? process.env["CWM_BASE_URL"] ?? "http://localhost:5000";
  const apiKey = opts.apiKey ?? stored.apiKey ?? process.env["CWM_API_KEY"] ?? "";
  return { baseUrl, apiKey };
}

// sdk/typescript/src/client.ts
var CwmError = class extends Error {
...
var CwmClient = class {
  ...
  async rlStep(envId, action) {
    return this.request(
      "POST",
      `/api/rl/environments/${envId}/step`,
      { action }
    );
  }
  async rlReset(envId) {
    return this.request(
      "POST",
      `/api/rl/environments/${envId}/reset`
    );
  }
  async getRlObservation(envId) {
    return this.request(
      "GET",
      `/api/rl/environments/${envId}/observation`
    );
  }
  // ------------------------------------------------------------------ //
  // Chaos Engineering                                                    //
  ...
  async getMulticloudResults(jobId) {
    return this.request(
      "GET",
      `/api/multi-cloud/jobs/${jobId}/results`
    );
  }
  // ------------------------------------------------------------------ //
  // API Key Management                                                   //
...
var CLI_ACTION_ALIASES = {
  scale_up: "scale_out",
  scale_down: "scale_in",
  no_op: "adjust_threshold"
};
var VALID_CLI_ACTIONS = [
  "scale_up",
  "scale_down",
  "no_op",
  "scale_out",
...
function resolveRlAction(raw) {
  const type = CLI_ACTION_ALIASES[raw] ?? raw;
  return { type, parameters: {} };
}
var program2 = new Command();
program2.name("cwm").description("Cloud World Model CLI \u2014 interact with the CWM simulation API").version("1.0.0").option("--base-url <url>", "CWM server URL (overrides config)").option("--api-key <key>", "API key (overrides config)").option("--output <format>", "Output format: text | json", "text");
var config = new Command("config").description("Manage CLI configuration");
program2.addCommand(config);
config.command("set").description("Save connection settings to ~/.cwm/config.json").option("--base-url <url>", "CWM server base URL").option("--api-key <key>", "API key").action((opts) => {
  const updates = {};
...
config.command("show").description("Print current configuration").action((opts, cmd) => {
...
var simulate = new Command("simulate").description("Simulation management");
program2.addCommand(simulate);
simulate.command("create").description("Create a simulation from a JSON spec file").requiredOption("--file <path>", "Path to simulation spec JSON").action(async (opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  const spec = readFile(opts["file"]);
  try {
...
simulate.command("get <id>").description("Fetch the current state of a simulation").action(async (id, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
simulate.command("list").description("List all simulations").action(async (opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
simulate.command("step <id>").description("Advance the simulation one tick and print metrics").option("--traffic <rps>", "RPS to inject", parseFloat).action(async (id, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
var rl = new Command("rl").description("RL environment management");
program2.addCommand(rl);
rl.command("create <simulation-id>").description("Create a Gym-compatible RL environment for a simulation").option("--max-steps <n>", "Episode length (default: 300)", parseInt).action(async (simulationId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
    const result = await makeClient({ baseUrl, apiKey }).createRlEnvironment(simulationId, {
      episodeConfig: opts["maxSteps"] ? { maxSteps: opts["maxSteps"] } : void 0
    });
    if (output === "json") {
      printJson(result);
    } else {
...
rl.command("step <env-id> <action>").description("Execute one RL step. Action: scale_up | scale_down | no_op | scale_out | scale_in | adjust_threshold").action(async (envId, actionRaw, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
...
rl.command("reset <env-id>").description("Reset an RL environment to start a new episode").action(async (envId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
var chaos = new Command("chaos").description("Chaos engineering");
program2.addCommand(chaos);
chaos.command("scenarios").description("List all built-in chaos scenarios").action(async (opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
chaos.command("run <simulation-id>").description("Run a chaos experiment against a simulation").option("--scenario <id>", "Built-in scenario ID (e.g. zone_failure)").option("--duration <ticks>", "Experiment duration in ticks", parseInt).option("--webhook-url <url>", "HTTPS URL for completion callback").action(async (simulationId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
    const result = await makeClient({ baseUrl, apiKey }).runChaos(simulationId, {
    ...
    if (output === "json") {
      ...
      console.log(`Chaos job started: ${result.jobId}`);
      console.log(`  Poll with: cwm chaos status ${result.jobId}`);
      console.log(`  Results:   cwm chaos results ${result.jobId}`);
    }
  } catch (err) {
    die(err);
  }
});
chaos.command("status <job-id>").description("Check the status of a chaos job").action(async (jobId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
chaos.command("results <job-id>").description("Fetch full results of a completed chaos job").action(async (jobId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
var multicloud = new Command("multicloud").description("Multi-cloud strategy exploration");
program2.addCommand(multicloud);
multicloud.command("explore").description("Start a multi-cloud strategy exploration job").requiredOption("--file <path>", "Path to workload profile JSON").option("--webhook-url <url>", "HTTPS URL for completion callback").action(async (opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  const workload = readFile(opts["file"]);
  try {
    const result = await makeClient({ baseUrl, apiKey }).exploreMulticloud(workload, {
      webhookUrl: opts["webhookUrl"]
    });
    if (output === "json") {
      printJson(result);
    } else {
      console.log(`Multi-cloud job started: ${result.jobId}`);
      console.log(`  Poll with: cwm multicloud status ${result.jobId}`);
      console.log(`  Results:   cwm multicloud results ${result.jobId}`);
    }
  } catch (err) {
    die(err);
  }
});
multicloud.command("status <job-id>").description("Check the status of a multi-cloud exploration job").action(async (jobId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
multicloud.command("results <job-id>").description("Fetch ranked strategies from a completed multi-cloud job").action(async (jobId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
var keys = new Command("keys").description("API key management");
program2.addCommand(keys);
keys.command("create").description("Create a new API key (requires admin scope)").requiredOption("--name <name>", "Human-readable key label").option("--scopes <scopes>", "Comma-separated scopes (e.g. read,write)", "read,write").option("--expires-at <iso>", "ISO-8601 expiry datetime").action(async (opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  const scopes = opts["scopes"].split(",").map((s) => s.trim());
  try {
    const result = await makeClient({ baseUrl, apiKey }).createApiKey(
      opts["name"],
      { scopes, expiresAt: opts["expiresAt"] }
    );
    if (output === "json") {
      printJson(result);
    } else {
...
keys.command("list").description("List all API keys (requires admin scope)").action(async (opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
...
keys.command("revoke <key-id>").description("Revoke an API key (requires admin scope)").action(async (keyId, opts, cmd) => {
  const { baseUrl, apiKey, output } = globalOpts(cmd);
  try {
